...
from models.ssd_model import DataTransform
...
class SSDPredictShow():
    ...
    def show(self, image_file_path, result_path, data_confidence_level):
        """
        物体検出の予測結果を表示をする関数。
        ...
        rgb_img, c_count, b_count = self.vis_bbox(rgb_img, bbox=predict_bbox, label_index=pre_dict_label_index,
                      scores=scores, label_names=self.eval_categories)

        self.save(rgb_img, result_path)

        return c_count, b_count
    ...
    def vis_bbox(self, rgb_img, bbox, label_index, scores, label_names):
        ...
        for i, bb in enumerate(bbox):
            ...
            if label_name == 'can':
                c_count = c_count + 1
            elif label_name == 'pet bottle':
                b_count = b_count + 1
        return rgb_img, c_count, b_count


    def save(self, rgb_img, result_path):
